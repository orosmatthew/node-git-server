...
import { parseGitName, createAction, infoResponse, basicAuth, noCache } from './util';
...
export class Git extends EventEmitter implements GitEvents {
  ...
  */
	constructor(repoDir: string | ((dir?: string) => string), options: GitOptions = {}) {
		...
		if (typeof repoDir === 'function') {
			...
			this.dirMap = (dir?: string): string => {
				return path.normalize((dir ? path.join(repoDir, dir) : repoDir) as string);
			};
		}
	...
	list(): Promise<string[]>;
	list(callback?: (error: Error | undefined, repos?: string[]) => void): Promise<string[]> | void {
	...
	listen(port: number, options?: GitServerOptions, callback?: () => void): Git {
		...
		const createServer =
			options.type == 'http' ? http.createServer : https.createServer.bind(this, options);
