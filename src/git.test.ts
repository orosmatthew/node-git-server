...
describe('git', () => {
	test('create, push to, and clone a repo', async () => {
		...
		await wrapCallback((callback: () => void) => {
			spawn('git', ['push', 'http://localhost:' + port + '/xyz/doom', 'master'], {
				cwd: srcDir
			}).on('exit', (code) => {
				expect(code).toBe(0);
				callback();
	...
	test('clone into programatic directories', async () => {
		...
		await wrapCallback((callback: () => void) => {
			spawn('git', ['push', 'http://localhost:' + port + '/doom.git', 'master'], {
				cwd: srcDir
			}).on('exit', (code) => {
				expect(code).toBe(0);
				callback();
	...
	test('test tagging', async () => {
		...
		await wrapCallback((callback: () => void) => {
			spawn('git', ['push', '--tags', 'http://localhost:' + port + '/doom', 'master'], {
				cwd: srcDir
			}).on('exit', (code) => {
				expect(code).toBe(0);
				callback();
	...
	test('should be able to send custom messages to git client (main stream)', async () => {
		...
		await wrapCallback((callback: () => void) => {
			const logs: any[] = [];
			const push = spawn('git', ['push', 'http://localhost:' + port + '/doom.git', 'master'], {
				cwd: srcDir
			});
